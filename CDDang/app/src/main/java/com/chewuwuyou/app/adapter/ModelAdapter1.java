...
public class ModelAdapter1 extends BaseAdapter {
	...
	public ModelAdapter1(Context context, List<CarModel> datas) {
		// mDatas = datas;
		mContext = context;
...
//					System.out.println(model.getFac() + "=====" + name);
				}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = LayoutInflater.from(mContext).inflate(
					R.layout.select_model_item, null);
		...
		// + bitmap);
		// ((ImageView) imageView).setImageDrawable(new BitmapDrawable(bitmap));
		// }
		// });
		// if (bitmap != null) {
		// hoder.textImage.setImageDrawable(new BitmapDrawable(bitmap));
		// }
		ImageUtils.displayImage(mDatas.get(position).getIcon(),
				hoder.textImage, 10);
