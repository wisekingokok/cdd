	...
	private void initBitmap(Canvas canvas) {
		if (sourceBitmap != null) {
			...
			if (bitmapWidth > width || bitmapHeight > height) {
				...
				float ratio_w = width / (bitmapWidth * 1.0f);
				float ratio_h = height / (bitmapHeight * 1.0f);
				if(ratio_w < ratio_h) {
					//横向放大
					currentBitmapWidth = width;
					currentBitmapHeight = ratio_w * bitmapHeight;
					totalRatio = initRatio = ratio_w;
				} else {
					//纵向放大
					currentBitmapHeight = height;
					currentBitmapWidth = width * ratio_h;
					totalRatio = initRatio = ratio_h;
					
				}
				
				float translateX = (width - currentBitmapWidth) / 2f;
				float translateY = (height - currentBitmapHeight) / 2f;
				// 先按照已有的缩放比例对图片进行缩放
				matrix.postScale(totalRatio, totalRatio);
				// 再根据移动距离进行偏移
				matrix.postTranslate(translateX, translateY);
