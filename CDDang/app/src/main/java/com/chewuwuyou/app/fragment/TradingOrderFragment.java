...
public class TradingOrderFragment extends BaseFragment {
	...
	private ArrayList<BarEntry> mBarArrayList = new ArrayList<BarEntry>();// 日成交订单集合

	private ArrayList<BarEntry> mMonthArrayList = new ArrayList<BarEntry>();// 月成交订单数
	private BusStatis mBusStati;// 订单单项统计
	private int mJinRichengjiao = 0, mZuoRichengjiao = 0;
	private int mBenYueChengJiao = 0, mShangYueChengJiao = 0;
	private JSONArray mJsonArray;// 所有城市数据
				...
				case Constant.NET_DATA_SUCCESS:
					...
					try {
						...
						mBarTwoDescTV.setText(jo.getString("myZone")
								+ "代理各区域成交订单统计");
						mJinRichengjiao = jo.getInt("jinRiChengJiao");
						mZuoRichengjiao = jo.getInt("zuoRiChengJiao");
						...
						mBenYueChengJiao = jo.getInt("benYueChengJiao");
						mShangYueChengJiao = jo.getInt("shangYueChengJiao");
						...
						for (int i = 0; i < jo.getJSONArray("zones").length(); i++) {// 区域订单统计
							...
							mBusStati.setOrderNumOne(zone
									.getString("thisZoneBenYueChengJiao"));
							mBusStati.setOrderNumTwo(zone
									.getString("thisZoneShangYueChengJiao"));
	...
	private BarData getTwoBarData(int count, ArrayList<BarEntry> yValues) {
		...
		barDataSet.setColor(getResources().getColor(R.color.canvas_yellow));
		barDataSet.setBarSpacePercent(70f);// 设置空白,缩小柱状图的宽度
		ArrayList<BarDataSet> barDataSets = new ArrayList<BarDataSet>();
		barDataSet.setValueFormatter(new ValueFormatter() {//对数据类型进行转换
