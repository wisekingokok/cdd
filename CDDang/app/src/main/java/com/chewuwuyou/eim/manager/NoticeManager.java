...
public class NoticeManager {
    ...
    public static NoticeManager getInstance(Context context) {
        ...
        if (noticeManager == null) {
            noticeManager = new NoticeManager(context.getApplicationContext());
        }
    ...
    public long saveNotice(Notice notice) {
        ...
        contentValues.put("friend_add_state", 0);


        st.deleteByCondition("im_notice", "notice_to=? and notice_from=? and type=?", new String[]{StringUtil.doEmpty(notice.getTo()), StringUtil.doEmpty(notice.getFrom()), String.valueOf(Notice.ADD_FRIEND)});
    ...
    public List<Notice> getUnReadNoticeList() {
        ...
        List<Notice> list = st.queryForList(new RowMapper<Notice>() {
            ...
            public Notice mapRow(Cursor cursor, int index) {
                ...
                notice.setAddState(cursor.getInt(cursor.getColumnIndex("friend_add_state")));
                return notice;
            }
     ...
     * @update 2012-5-16 下午3:22:44
     */
    public void updateAddFriendStatus(String id, Integer status, String content, Integer addState) {
        SQLiteTemplate st = SQLiteTemplate.getInstance(manager, false);
        ContentValues contentValues = new ContentValues();
        contentValues.put("status", status);
        contentValues.put("content", content);
        contentValues.put("friend_add_state", addState);
        st.updateById("im_notice", id, contentValues);
    }
    ...
    public Integer getUnReadNoticeCount() {
        SQLiteTemplate st = SQLiteTemplate.getInstance(manager, false);
        return st.getCount("select _id from im_notice where status=?", new String[]{"" + Notice.UNREAD});
    }
    ...
    public List<Notice> getUnReadNoticeListByType(int type) {
        ...
        String notice_to = CacheTools.getUserData("userId") + "@iz232jtyxeoz";
        String[] str = new String[]{"" + Notice.UNREAD, "" + type};
        /**
         * notice_to有事会丢掉Smack字符串,故使用like
         */
        String sql = "select * from im_notice where status=? and type=? and notice_to like '" + notice_to + "%' order by notice_time desc";//修改为加我为好友的提醒,并进行排序
        SQLiteTemplate st = SQLiteTemplate.getInstance(manager, false);
        List<Notice> list = st.queryForList(new RowMapper<Notice>() {
    ...
    public Integer getUnReadNoticeCountByTypeAndFrom(int type, String from) {
        SQLiteTemplate st = SQLiteTemplate.getInstance(manager, false);
        return st.getCount("select _id from im_notice where status=? and type=? and motice_from=?", new String[]{"" + Notice.UNREAD, "" + type, from});
    }
    ...
    public void updateStatusByFrom(String xfrom, Integer status) {
        ...
        st.update("im_notice", values, "notice_from=?", new String[]{"" + xfrom});
    }
     ...
     */
    public List<Notice> getNoticeListByTypeAndPage(int type, int isRead, int pageNum, int pageSize) {
        int fromIndex = (pageNum - 1) * pageSize;
        StringBuilder sb = new StringBuilder();
        String[] str = null;
        sb.append("select * from im_notice where type=?");
        if (Notice.UNREAD == isRead || Notice.READ == isRead) {
            str = new String[]{"" + type, "" + isRead, "" + fromIndex, "" + pageSize};
