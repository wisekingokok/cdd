...
public class SegmentControl extends View {
	...
	public SegmentControl(Context context, AttributeSet attrs, int defStyle) {
		...
		mTextSize = ta.getDimensionPixelSize(
				R.styleable.SegmentControl_android_textSize, (int) TypedValue
		...
		mCornerRadius = ta.getDimensionPixelSize(
				R.styleable.SegmentControl_cornerRadius, (int) TypedValue
						.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, context
								.getResources().getDisplayMetrics()));
		mDirection = Direction.values()[ta.getInt(
				R.styleable.SegmentControl_direction, 0)];

		mHorizonGap = ta.getDimensionPixelSize(
				R.styleable.SegmentControl_horizonGap, 0);
		mVerticalGap = ta.getDimensionPixelSize(
				R.styleable.SegmentControl_verticalGap, 0);

		int gap = ta.getDimensionPixelSize(R.styleable.SegmentControl_gaps,
		...
		mSelectedDrawable = new RadiusDrawable(mCornerRadius, false,
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mTexts != null && mTexts.length > 0) {
			...
			// mChildrenWidth = mDirection == Direction.HORIZON ?
			// mSingleChildWidth * mTexts.length : mSingleChildWidth;
			// mChildrenHeight = mDirection == Direction.VERTICAL ?
			// mSingleChildHeight * mTexts.length : mSingleChildHeight;
		} else {
		...
		if (mTexts != null && mTexts.length > 0) {
			for (int i = 0; i < mTexts.length; i++) {
				...
				if (i == mCurrentIndex && mSelectedDrawable != null) {
					...
					mSelectedDrawable
							.setRadiuses(topLeftRadius, topRightRadius,
				...
				canvas.drawText(
						...
						mCacheBounds[i].left
								...
								/ 2,
						mCacheBounds[i].top
								+ ((mSingleChildHeight + mTextBounds[i]
										.height()) / 2), mPaint);
			}
		}
