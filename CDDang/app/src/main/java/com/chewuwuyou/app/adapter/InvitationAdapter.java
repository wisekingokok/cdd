...
public class InvitationAdapter extends BaseAdapter implements OnClickListener {

	private List<TieItem> mData;
	private Handler mHandler;
	private static final int DING = 0;
	private static final int OTHER = DING + 1;
	private int mTuWidth = 0;
	...
	public InvitationAdapter(Activity context, Handler handler,
			List<TieItem> data, String action) {
	...
	public void setData(List<TieItem> mData) {
		this.mData = mData;
		notifyDataSetChanged();
	}

	public InvitationAdapter(Activity context, Handler handler,
			List<TieItem> data, String banUrl, int todayNum) {
		this.context = context;
		this.mData = data;
		this.mBanUrl = banUrl;
	...
	private View createView(View convertView, int position) {
		...
		if (position == 0 && mAction == null) {
			...
			ImageUtils.displayImage(mBanUrl, viewCache.getmTieBanKuaiIV(), 0,
					R.drawable.image_default, R.drawable.image_load_fail);
			viewCache.getmTieBanKuaiNumberTV().setText(
					String.valueOf(mTodayNum));
		} else {
		...
		if (tie != null) {
			ImageUtils.displayImage(tie.getUrl(),
					viewCache.getmTieItemAvatarIV(), 360,
					R.drawable.user_yuan_icon, R.drawable.user_yuan_icon);
			viewCache.getmTieItemNameTV().setText(
					CarFriendQuanUtils.showCarFriendName(tie));
			if (tie.getSex() == 0) {
				...
				viewCache.getmTieItemSexIV().setImageResource(
						R.drawable.icon_nosex);
			}
			// viewCache.getmTieItemSexIV().setImageResource(tie.getSex() == 1 ?
			// R.drawable.woman : R.drawable.man);
			viewCache.getmTieItemAreaTV().setText(
					!TextUtils.isEmpty(tie.getLocation()) ? tie.getLocation()
							: "不告诉你");
			...
			if (tus.size() > 0) {
				...
				// +
				// context.getResources().getDimensionPixelSize(R.dimen.quan_tu_interval)
				// * (tuUrls.size() - 1);
				...
				final LinearLayout tieItemTusInnerLL = viewCache
						.getmTieItemTusInnerLL();
				for (int i = 0; i < tus.size(); i++) {
					final ImageView tuIV = new ImageView(context);
					...
					iv_params = new LinearLayout.LayoutParams(new LayoutParams(
							mTuWidth, mTuWidth));
					// } else {
					// iv_params = new LinearLayout.LayoutParams(new
					// LayoutParams(mTuWidth, mTuHeight));
					// }
					...
					ImageUtils.displayImage(url, tuIV, 0,
			...
			Date date = null;
			try {
				date = new SimpleDateFormat("yyyyMMddHHmmss").parse(tie
						.getPublishTime());
			} catch (ParseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			viewCache.getmTieItemDateTV().setText(
			...
			// View.GONE);
			// 后面要改的
			viewCache.getmTieItemZanTV().setText(
					context.getResources().getString(
							R.string.item_zan_ping_cnt, tie.getTiezancnt()));
			viewCache.getmTieItemPingTV().setText(
					context.getResources().getString(
							R.string.item_zan_ping_cnt, tie.getTiepingcnt()));

			if (Integer.parseInt(tie.getZaned()) == 1) {
				viewCache.getmTieItemZanTV()
						.setCompoundDrawablesWithIntrinsicBounds(
								R.drawable.iconheart, 0, 0, 0);
			} else {
				viewCache.getmTieItemZanTV()
						.setCompoundDrawablesWithIntrinsicBounds(
								R.drawable.iconhear, 0, 0, 0);
			}
	...
	public void onClick(View v) {
		...
		case R.id.tie_item_ll:
			// 显示大图
			intent = new Intent(context, TieDetailActivity.class);
		...
		case R.id.tie_item_avatar_iv:
			// 进入到个人详情中
			intent = new Intent(context, PersonalHomeActivity2.class);
