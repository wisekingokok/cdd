...
import com.chewuwuyou.app.adapter.TieDetailAdapter;
...
public class TieDetailActivity extends CDDBaseActivity implements
	...
	// 去评
	@ViewInject(id = R.id.tie_detail_to_ping_ll, click = "onAction")
	...
	private TieDetailAdapter mAdapter;
	...
	public static final int REPLY_TIE_PING = 113;// 回复一个评论
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case REPLY_PING:// 评价一个PING
				final TiePingItem ping = (TiePingItem) msg.obj;
			...
			case EDIT_TIE:
				...
				ActionSheet menuView = new ActionSheet(TieDetailActivity.this);
					...
					public void onItemClick(int itemPosition) {
						...
						case 0:
							if (mTieZiji.equals("1")) {// 自己发的帖子
								...
								new AlertDialog.Builder(TieDetailActivity.this)
										.setTitle("请输入举报原因")
										.setView(editText)
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tie_detail_layout);
	...
	protected void initView() {
		mPullToRefreshListView = (PullToRefreshListView) findViewById(R.id.tie_detail_lv);
		mExpandedImageViewPager = (HackyViewPager) findViewById(R.id.tie_detail_expanded_image_viewpager);
	...
	protected void initData() {
		...
		// mFaceAdapter = new FaceAdapter(TieDetailActivity.this, mHandler);
		// mToPingFaceGV.setAdapter(mFaceAdapter);
		staticFacesList = ExpressionUtil.initStaticFaces(this);// 初始化表情图
		initFaceViewPager();// 初始化表情
		ExpressionUtil.getFaceStrMap(TieDetailActivity.this);// 获取新的表情数据Map
		if (mData == null) {
			mData = new ArrayList<Object>();
		}
		if (mAdapter == null) {
			mAdapter = new TieDetailAdapter(TieDetailActivity.this, mHandler,
					mData, mExpandedImageViewPager, mContainer);
		}
	...
	private void showDeletePingDialog(final TiePingItem ping) {
		mDeletePingDialog = new AlertDialog.Builder(TieDetailActivity.this)
				.setTitle("确定删除")
	...
	private void getTieDetail(final boolean refresh) {
		...
		requestNet(new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				if (!mIsSetEmptyTV) {
					TextView tv = new TextView(TieDetailActivity.this);
				...
				case Constant.NET_DATA_SUCCESS:
					...
					if (refresh) {
						...
						mCurcor = response.getTieping().size();
						if (response.getTieping().size() < Constant.TIE_DETAIL_PAGE_SIZE) {
	...
	private void delTiePing(final TiePingItem tiePing) {
		...
		requestNet(new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				switch (msg.what) {
				case Constant.NET_DATA_SUCCESS:
					Toast.makeText(TieDetailActivity.this, "删除成功",
					...
					break;
				case Constant.NET_DATA_FAIL:
					Toast.makeText(TieDetailActivity.this, "删除失败",
							Toast.LENGTH_SHORT).show();
					break;
				default:
					break;
	...
	private void toggleZan() {
		...
		requestNet(new Handler() {
			...
			public void handleMessage(Message msg) {
					...
					if (tieZan != null) {
						...
						if (((TieDetailHeaderEntity) mData.get(0)).getZaned()
							...
							((TieDetailHeaderEntity) mData.get(0)).getTiezan()
									.add(tieZan);
							Toast.makeText(TieDetailActivity.this, "点赞成功",
									Toast.LENGTH_SHORT).show();
						} else {
							((TieDetailHeaderEntity) mData.get(0))
									.setZaned(0 + "");
							...
							Toast.makeText(TieDetailActivity.this, "取消赞",
									Toast.LENGTH_SHORT).show();
						}
				...
				case Constant.NET_DATA_FAIL:
					// 提示切换赞失败
					Toast.makeText(TieDetailActivity.this, "失败，请重新尝试",
							Toast.LENGTH_SHORT).show();
					break;
				default:
					break;
	...
	private void pingTie() {
		...
		requestNet(new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				switch (msg.what) {
				case Constant.NET_DATA_SUCCESS:
					Toast.makeText(TieDetailActivity.this, "评论成功",
							Toast.LENGTH_SHORT).show();
					...
					break;
				case Constant.NET_DATA_FAIL:
					Toast.makeText(TieDetailActivity.this, "评论失败",
							Toast.LENGTH_SHORT).show();
					break;
				default:
					break;
	...
	private void showDeleteTieDialog(final int tieZiId) {
		mDeleteTieDialog = new AlertDialog.Builder(TieDetailActivity.this)
				.setTitle("确定删除")
				.setPositiveButton("确定", new DialogInterface.OnClickListener() {
